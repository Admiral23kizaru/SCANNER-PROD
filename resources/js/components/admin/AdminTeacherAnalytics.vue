  ...
  <section class="rounded-lg border border-slate-200 bg-white shadow-sm">
    <div class="flex flex-col gap-3 border-b border-slate-200 p-4">
      <div>
        ...
        <p class="text-sm text-slate-500">School-scoped teacher distribution by advisory, position, or status.</p>
      </div>
      <div class="grid gap-3 md:grid-cols-4">
        <label class="text-xs font-semibold uppercase tracking-wide text-slate-500 md:col-span-2">
          Search
          <input v-model="search" type="search" placeholder="Teacher, HRID, advisory" class="mt-1 w-full rounded-md border border-slate-300 px-3 py-2 text-sm font-normal normal-case tracking-normal text-slate-900 outline-none focus:border-blue-500" />
        </label>
    ...
    <div class="grid gap-5 p-4 xl:grid-cols-[520px_1fr]">
      <div class="rounded-lg border border-slate-200 bg-gradient-to-br from-white to-slate-50 p-4">
        <div class="mb-2 flex items-center justify-between gap-3">
          ...
          <p class="rounded-full bg-violet-50 px-2 py-1 text-xs font-semibold text-violet-700">{{ chartRows.length }} group(s)</p>
        </div>
      ...
      <div class="grid content-start gap-2 sm:grid-cols-2">
        ...
        <div class="rounded-lg border border-slate-200 bg-slate-50 px-3 py-3">
          <p class="text-xs font-semibold uppercase tracking-wide text-slate-500">Advisory Assigned</p>
          <p class="mt-1 text-2xl font-black text-slate-950">{{ advisoryAssigned }}</p>
        </div>
        <button v-for="(row, index) in chartRows" :key="row.label" type="button" class="flex items-center justify-between rounded-lg border px-3 py-2 text-left text-sm transition hover:-translate-y-0.5 hover:shadow-sm" :class="selectedLabel === row.label ? 'border-violet-300 bg-violet-50' : 'border-slate-200 bg-white'" @click="selectedLabel = selectedLabel === row.label ? '' : row.label">
    ...
    <div class="border-t border-slate-200 p-4">
      <div class="rounded-lg border border-slate-200 bg-white p-4">
        <p class="text-xs font-bold uppercase tracking-wide text-slate-500">Teacher Advisory / Position Summary</p>
        <div class="mt-3 h-80">
          <Bar v-if="chartRows.length" :data="barData" :options="barOptions" />
...
import { fetchTeachers } from '../../services/adminService';
...
const groupBy = ref('advisory');
const filters = reactive({ grade: '' });
const colors = ['#7c3aed', '#2563eb', '#16a34a', '#f59e0b', '#dc2626', '#0891b2', '#ea580c', '#475569', '#65a30d', '#db2777'];

const filteredRows = computed(() => {
  const term = search.value.trim().toLowerCase();
  return rows.value.filter((row) => {
    const advisory = advisoryLabel(row);
    const matchesSearch = !term || [row.name, row.email, row.employee_id, row.job_title, row.status, advisory].join(' ').toLowerCase().includes(term);
    const matchesGrade = !filters.grade || gradeLabel(row.grade_level) === filters.grade;
    return matchesSearch && matchesGrade;
  });
});

const gradeOptions = computed(() => uniqueSorted(rows.value.map((row) => gradeLabel(row.grade_level)).filter((grade) => grade !== 'No advisory')));
const advisoryAssigned = computed(() => filteredRows.value.filter((row) => advisoryLabel(row) !== 'No advisory').length);
const chartRows = computed(() => groupedRows(filteredRows.value, teacherGroupLabel).sort((a, b) => b.value - a.value || a.label.localeCompare(b.label)));
const chartTotal = computed(() => chartRows.value.reduce((sum, row) => sum + row.value, 0));
const groupLabel = computed(() => (({ advisory: 'by advisory', position: 'by position', status: 'by status' })[groupBy.value] || 'current filter'));
...
const chartOptions = computed(() => ({
  ...
  cutout: '64%',
  plugins: { legend: { display: false }, tooltip: { callbacks: { label: (context) => ` ${context.label}: ${context.raw} teacher(s), ${chartTotal.value ? Math.round((Number(context.raw) / chartTotal.value) * 100) : 0}%` } } },
}));

const barData = computed(() => ({
  labels: chartRows.value.map((row) => row.label),
  datasets: [{ label: 'Teachers', data: chartRows.value.map((row) => row.value), backgroundColor: '#7c3aed', borderRadius: 8, maxBarThickness: 36 }],
}));
...
function teacherGroupLabel(row) {
  if (groupBy.value === 'position') return row.job_title || row.designation || 'Unspecified position';
  if (groupBy.value === 'status') return row.status || 'Unspecified status';
  return advisoryLabel(row);
}

function advisoryLabel(row) {
  const grade = gradeLabel(row.grade_level);
...
async function loadRows() {
  ...
  error.value = '';
  try {
    const first = await fetchTeachers({ page: 1, per_page: 100 });
    const all = [...(first.data || [])];
    for (let page = 2; page <= Number(first.last_page || 1); page += 1) {
      const next = await fetchTeachers({ page, per_page: 100 });
      all.push(...(next.data || []));
    }
    rows.value = all;
  } catch (err) {
